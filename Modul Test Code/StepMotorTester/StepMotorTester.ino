...
int button = 6; // push button for user
int st = 2; // step motor driver step pin
int led[5] = {5,6,9,10,11}; // pins for white LEDs
int sensor = 8; // infarot sensor (for positioning) pin, using the white wire, 1-position hole, 0-normal.
...
long timeInterval = 5000;//(2 * 60 * 1000); // When machine is not used more than 2 minute, it will go back to start.
long caliSpeed = 2000; //Unit: Microsecond. set the rotation speed when motor do calibration.
...
void timeControler(boolean buttonState, boolean sensorState, long timeInterval){  
...
int readPin[4] = {3,4,7,12}; //pin map that connect to the codier
